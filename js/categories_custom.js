	        ...
	        filterButton.on('click', function()
	        {
	        	$('.product-grid').isotope({
		            filter: function()
		            {
			        	...
			        	var priceMin = parseFloat(priceRange.split('-')[0].replace('MAD', ''));
			        	var priceMax = parseFloat(priceRange.split('-')[1].replace('MAD', ''));
			        	var itemPrice = $(this).find('.product_price').clone().children().remove().end().text().replace( 'MAD', '' );
